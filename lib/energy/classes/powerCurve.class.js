        ...
        for (let currentYearCurveDays of classifiedByYears) {
            if (currentYearCurveDays.year && currentYearCurveDays.days) {
                ...
                for (let month = startDateMonth; month <= endDateMonth; month++) {
                    ...
                    const firstDateOfMonth = new Date(currentYearCurveDays.year, month, 1);
                    const lastDateOfMonth = new Date(currentYearCurveDays.year, month + 1, 0);
                    //filter by those dates and asign it to the month index
                    result[currentYearCurveDays.year][month + 1] = new PowerCurve(new PowerCurve(currentYearCurveDays.days, false, "", true).filterByDates(firstDateOfMonth, lastDateOfMonth), false, "", true);
                }
            }
        }
        return result;
