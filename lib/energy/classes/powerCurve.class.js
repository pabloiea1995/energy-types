...
class PowerCurve {
    constructor(daysList, ignore0, identifier, parseDate) {
        this.days = daysList;
    ...
    convertoToSerie() {
        ...
        for (let dia in this.days) {
            ...
            for (let i = 1; i < 26; i++) {
                ...
                if (this.ignore0) {
                    if (currentDay.valuesList[i.toString()] !== 0 &&
                        ...
                        if (this.parseDate) {
                            ...
                            x.push(new Date(parseInt(year), parseInt(month) - 1, parseInt(day), i - 1));
            ...
            xDailyAverage.push(`${date.getDate() < 10 ? "0" + date.getDate() : date.getDate()}/${date.getMonth() + 1 < 10
                ? "0" + (date.getMonth() + 1)
                : date.getMonth() + 1}`);
    ...
    aggregatePowerCurve(curveToAggregate) {
        const curve = this.days;
        let resultCurve = [];
        resultCurve = curve.map((day) => {
            let resultDay = { date: day.date };
            let aggregatedValuesList = JSON.parse(JSON.stringify(day.valuesList)) || {};
            if (aggregatedValuesList && day.valuesList) {
                Object.keys(day.valuesList).forEach((hour) => {
                    if (hour) {
                        const dayToAggregate = curveToAggregate.days.filter((dayToAggregate) => day.date === dayToAggregate.date)[0];
        ...
        return new PowerCurve(resultCurve, this.ignore0, this.identifier + " + " + curveToAggregate.identifier, this.parseDate);
    }
    /**
    ...
    calculateStatistics(periodDistribution) {
                    ...
                    Object.keys(valuesList).map((hourS) => {
                        ...
                        if (!isNaN(consumoHorario)) {
                            ...
                            if (periodDistribution) {
                                ...
                                dayPeriodDistribution = periodDistribution.filter((periodDay) => periodDay.date.split("T")[0] === date)[0];
        ...
        Object.keys(weeklyAverageConsumption).map((year) => {
            ...
            Object.keys(weeklyAverageConsumption[yearI]).map((weekday) => {
                weeklyAverage[year][weekday] = parseFloat((weeklyAverageConsumption[yearI][weekday].acumulado /
                    weeklyAverageConsumption[yearI][weekday].cuenta).toFixed(6));
            });
        });
        let hourlyAverage = {};
        //calculamos la media de los consumos diariso medios semanales
        ...
        Object.keys(hourlyAverageConsumption).map((year) => {
            ...
            Object.keys(hourlyAverageConsumption[yearI]).map((hour) => {
                hourlyAverage[year][hour] = parseFloat((hourlyAverageConsumption[yearI][hour].acumulado /
                    hourlyAverageConsumption[yearI][hour].cuenta).toFixed(6));
            });
        });
        ...
        Object.keys(weeklyhourlyAverageConsumption).map((year) => {
            ...
            Object.keys(weeklyhourlyAverageConsumption[yearI]).map((weekDay) => {
                ...
                Object.keys(weeklyhourlyAverageConsumption[yearI][weekDayI]).map((hour) => {
                    ...
                    weeklyhourlyAverageConsumption[yearI][weekDayI][hourI] = parseFloat((weeklyhourlyAverageConsumption[yearI][weekDayI][hourI]
                        .acumulado /
                        weeklyhourlyAverageConsumption[yearI][weekDayI][hourI].cuenta).toFixed(6));
                });
            });
        });
    ...
    applyMultiplier(multiplier) {
        ...
        return new PowerCurve(resultCurve, this.ignore0, this.identifier, this.parseDate);
    }
    /**
    ...
    aggregateByPeriod(periodDistribution) {
        let periodAggregation = {};
        try {
            if (periodDistribution[0].date) {
                let periodDistributionDay;
                ...
                this.days.forEach((day) => {
                    ...
                    if (periodDistribution[0].date.split("T").length > 0) {
                        ...
                        periodDistributionDay = periodDistribution.filter((periodDay) => periodDay.date.split("T")[0] === day.date)[0];
                    }
                    else {
                        //get the period distribution day
                        periodDistributionDay = periodDistribution.filter((periodDay) => periodDay.date === day.date)[0];
                    }
                    if (periodDistributionDay &&
                        day.valuesList &&
                        periodDistributionDay.valuesList) {
                        Object.keys(day.valuesList).forEach((hour) => {
                            //undefined values of the curve must betreated as 0
                            let currentValue = day.valuesList[hour] !== undefined ? day.valuesList[hour] : 0;
                            let currentPeriod = ("p" +
                                periodDistributionDay.valuesList[hour]);
                            if (currentPeriod) {
    ...
    rotateCurve(startingDate) {
        ...
        if (numDays) {
            ...
            const startingDayObject = this.days.find((day, index) => {
            ...
            if (startingDayObject && startingDateIndex !== undefined) {
                ...
                for (let rightIndex = startingDateIndex + 1; rightIndex < numDays; rightIndex++) {
                ...
                return new PowerCurve(rotatedDays, this.ignore0, "rotated curve-" + this.identifier, this.parseDate);
            }
        }
    }
    /**
     ...
     * @returns
     */
    alignPowerCurveDates(reference) {
        const alignedPowerCurve = new PowerCurve(this.days, this.ignore0, this.identifier, this.parseDate);
    ...
    classifyByYearsAndMonths() {
        ...
        for (let currentYearCurveDays of classifiedByYears) {
            if (currentYearCurveDays.year && currentYearCurveDays.days) {
                ...
                new PowerCurve(currentYearCurveDays.days, false, "", true).sortByDate(), false, "", true).getTotalPeriod();
                ...
                for (let month = startDateMonth; month <= endDateMonth; month++) {
                    ...
                    const firstDateOfMonth = new Date(currentYearCurveDays.year, month, 1, 12);
                    const lastDateOfMonth = new Date(currentYearCurveDays.year, month + 1, 0, 12);
                    //filter by those dates and asign it to the month index
                    result[currentYearCurveDays.year][month + 1] = new PowerCurve(new PowerCurve(currentYearCurveDays.days, false, "", true).filterByDates(new Date(firstDateOfMonth.toISOString().split("T")[0]), new Date(lastDateOfMonth.toISOString().split("T")[0])), false, "", true);
                }
            }
        }
        return result;
